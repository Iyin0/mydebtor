...
const Signup = () => {
    ...
    const schState = useRef();
    const schLga = useRef();
    const schLevel = useRef();
    const schPwd = useRef();
    const schCpwd = useRef();
    ...
    const signup = () => {
        if (schoolName === '') {
            ...
            schName.current.classList.add('signup-error');
        }
        else if (schoolEmail === '') {
            password_len.current.classList.add('password-err');
            password_text.current.innerText = "School email cannot be empty";
            schName.current.classList.remove('signup-error');
            schEmail.current.classList.add('signup-error');
        }
            ...
            schEmail.current.classList.remove('signup-error');
            schState.current.classList.add('signup-error');
        }
        else if (selectLga === 'Select') {
            password_len.current.classList.add('password-err');
            password_text.current.innerText = "Invalid school location";
            schState.current.classList.remove('signup-error');
            schLga.current.classList.add('signup-error');
        }
        else if (institution === 'Select') {
            password_len.current.classList.add('password-err');
            password_text.current.innerText = "Invalid school level";
            schLga.current.classList.remove('signup-error');
            schLevel.current.classList.add('signup-error');
        }
        else if (password.length < 8) {
            password_len.current.classList.add('password-err');
            password_text.current.innerText = "Your password must be up to 8 characters";
            schLevel.current.classList.remove('signup-error');
            schPwd.current.classList.add('signup-error');
        }
        else if (password !== cpassword) {
            password_len.current.classList.add('password-err');
            password_text.current.innerText = "The confirm password does not match your password";
            schPwd.current.classList.remove('signup-error');
            schCpwd.current.classList.add('signup-error');
        }
        else {
            password_len.current.classList.remove('password-err');
            schCpwd.current.classList.remove('signup-error');
            ...
            if (checkbox.current.checked === false) {
                ...
                setTimeout(() => navigate('/verify1'), 1000)
            }
        }
    }
        ...
        <div className='signup'>
            ...
            <div className="signup-form-container" >
                ...
                <form action="" className='signup-form'>
                    ...
                    <input type="text" className="signup-input" ref={schName} value={schoolName} onChange={(e) => setSchoolName(e.target.value)} /><br />
                    <label htmlFor="" className="signup-label">School Email Address</label><br />
                    <input type="text" className="signup-input" ref={schEmail} value={schoolEmail} onChange={(e) => setSchoolEmail(e.target.value)} /><br />
                    <div className="signup-location">
                        <div className="signup-location-selection">
                    ...
                    <div className="signup-input signup-pswd" ref={schPwd} >
                    ...
                    <div className="signup-input signup-pswd" ref={schCpwd} >
