import SearchBar from "./searchBar";
import SideNavbar from "./sideNavbar";
import { useEffect } from "react";
import { useDispatch } from "react-redux";
import { toggleSideNav } from "./store/landingDropdown";
import './css/currentDebtor.css';
import { all_debtors } from "./allDebtors";
import { Link, useNavigate } from "react-router-dom";


const CurrentDebtor = () => {

    const toggleSideNavbar = useDispatch();
    const navigate = useNavigate();

    useEffect(() => {
        toggleSideNavbar(toggleSideNav(false));
    }, [toggleSideNavbar])

    useEffect(() => {
        // 👇️ scroll to top on page load
        window.scrollTo({ top: 0, left: 0, behavior: 'smooth' });
    }, []);

    return (
        <div>
            <SideNavbar />
            <div className="current-debtor-page">
                <SearchBar />
                <div className="current-debtor">
                    <h1>Current Debtors</h1>
                    {all_debtors.map((debtor, index) => (
                        <Link to={`/current-debtors/debtor_${debtor.id}`} className="debtor" key={index}>
                            <div className="debt-left">
                                <div className="debt-left-top">
                                    <h2 className="debtor-name">{debtor.ward}</h2>
                                    <p className="debtor-email">{debtor.email}</p>
                                </div>
                                <p className="debtor-description"><span className="debtor-desc">Debt description: </span>{debtor.description}</p>
                            </div>
                            <div className="debt-right">
                                <div className="debt-right-top">
                                    <div className="debtor-owe">
                                        <svg width="60" height="61" viewBox="0 0 60 61" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <path d="M57.7717 34.3619H49.5813V24.7465H57.7717C59.0024 24.7465 60 23.5291 60 22.0255C60 20.5217 59.0024 19.3036 57.7717 19.3036H49.5813V5.96168C49.5813 4.01411 49.2663 2.58463 48.6452 1.71385C48.0549 0.885468 47.2789 0.5 46.2041 0.5C45.1786 0.5 44.4318 0.882809 43.8529 1.70603C43.2391 2.57768 42.9275 4.00982 42.9275 5.96256V19.3052H27.521L20.9703 7.1965C20.4097 6.11586 19.8825 5.1036 19.3706 4.13196C18.9097 3.25691 18.4602 2.54445 18.0356 2.01248C17.6597 1.5415 17.2313 1.177 16.7303 0.89684C16.2576 0.633215 15.6566 0.500886 14.9455 0.500886C14.0386 0.500886 13.2154 0.804386 12.4271 1.42793C11.647 2.04468 11.1078 2.80719 10.7785 3.75919C10.4894 4.66393 10.3364 6.05029 10.3364 7.83983V19.3043H2.22741C0.996973 19.3045 0 20.5226 0 22.0262C0 23.5298 0.996973 24.7472 2.22753 24.7472H10.3366V34.3637H2.22753C0.996973 34.3637 0 35.5815 0 37.0856C0 38.5888 0.996973 39.8057 2.22753 39.8057H10.3366V55.0388C10.3366 56.9287 10.6617 58.3438 11.3056 59.2429C11.918 60.1002 12.6944 60.4982 13.7488 60.4982C14.7661 60.4982 15.5365 60.0984 16.1754 59.2395C16.8296 58.3596 17.1605 56.9454 17.1605 55.0389V39.8057H31.1017L38.646 53.937C39.1706 54.8711 39.7122 55.814 40.2524 56.7385C40.7398 57.5687 41.2715 58.3002 41.8325 58.9112C42.3378 59.4638 42.8794 59.8688 43.4425 60.1159C44.0221 60.3708 44.6995 60.5 45.4513 60.5C47.4915 60.5 49.5811 59.737 49.5811 54.0787V39.8057H57.7715C59.0022 39.8057 59.9998 38.587 59.9998 37.0838C60 35.5806 59.0024 34.3619 57.7717 34.3619ZM42.9274 24.7463V34.3618H35.6682L30.4668 24.7463H42.9274ZM17.1606 13.6979L20.1542 19.3036H17.1606V13.6979ZM17.1606 34.3619V24.7465H23.061L28.1953 34.3619H17.1606ZM42.9274 47.7828L38.6132 39.8057H42.9274V47.7828Z" fill="#292929" />
                                        </svg>
                                        <p>{debtor.owes}</p>
                                    </div>
                                    <div className="owe-date">{debtor.date}</div>
                                </div>
                                <p className="view">View details</p>
                            </div>
                        </Link>
                    ))}
                    <button className="add-debtor" onClick={() => navigate('/current-debtors/debtor')}>
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M12 12.75C8.83 12.75 6.25 10.17 6.25 7C6.25 3.83 8.83 1.25 12 1.25C15.17 1.25 17.75 3.83 17.75 7C17.75 10.17 15.17 12.75 12 12.75ZM12 2.75C9.66 2.75 7.75 4.66 7.75 7C7.75 9.34 9.66 11.25 12 11.25C14.34 11.25 16.25 9.34 16.25 7C16.25 4.66 14.34 2.75 12 2.75Z" fill="#FEFEFE" />
                            <path d="M14.8248 15.1313L14.8248 15.1314L14.8276 15.1321C14.9581 15.168 15.0357 15.3005 14.9981 15.4374C14.9622 15.5677 14.8303 15.6453 14.6936 15.6082C13.8264 15.3651 12.9212 15.25 12.0002 15.25C7.51602 15.25 3.66016 18.1738 3.66016 22C3.66016 22.1339 3.54401 22.25 3.41016 22.25C3.2763 22.25 3.16016 22.1339 3.16016 22C3.16016 18.0893 7.03505 14.75 12.0002 14.75C12.9669 14.75 13.9112 14.8744 14.8248 15.1313Z" stroke="#FEFEFE" />
                            <path d="M19.4998 18.73H16.5098C16.0998 18.73 15.7598 18.39 15.7598 17.98C15.7598 17.57 16.0998 17.23 16.5098 17.23H19.4998C19.9098 17.23 20.2498 17.57 20.2498 17.98C20.2498 18.39 19.9098 18.73 19.4998 18.73Z" fill="#FEFEFE" />
                            <path d="M18 20.26C17.59 20.26 17.25 19.92 17.25 19.51V16.52C17.25 16.11 17.59 15.77 18 15.77C18.41 15.77 18.75 16.11 18.75 16.52V19.51C18.75 19.93 18.41 20.26 18 20.26Z" fill="#FEFEFE" />
                        </svg>
                        <p className="new-debtor">Add new debtor</p>
                    </button>
                </div>
            </div>
        </div>
    );
}

export default CurrentDebtor;